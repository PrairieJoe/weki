...
function focusSearchComposer(){ if(state.page!=="search"||document.querySelector("#onboarding-root"))return; const input=document.querySelector("#query"); if(!input)return; input.focus({preventScroll:true}); const end=input.value.length; input.setSelectionRange?.(end,end); }
...
render=function(){ baseRender(); syncBrandIcon(); syncSidebarVersion(); syncSettingsCardLayout(); syncSelectedFileList(); syncMockControls(); syncMyboxCredentialStatus(); syncMyboxCredentialEditor(); syncMoreResults(); onboardingController?.refreshTarget(); void syncRuntimeComponents(); focusSearchComposer(); };
