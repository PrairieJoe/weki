...
async function installAllRuntimeComponents(button){
  ...
  button.disabled=true;state.toast="고품질 검색 구성요소를 설치하는 중입니다…";render();
  startRuntimePolling();
  try{
    ...
    const watcher=await armRuntimeInstallAutoRestart({responseOk:response.ok,startedAt:data.startedAt,watch:window.wekiApp?.watchRuntimeInstall});
    state.toast=response.ok
      ?watcher.ok?"고품질 검색 구성요소 설치를 시작했습니다. 완료되면 앱이 자동으로 다시 시작됩니다.":"설치는 시작했지만 자동 재시작 감시를 연결하지 못했습니다. 완료 후 앱을 직접 다시 시작해 주세요."
...
render=function(){ baseRender(); syncBrandIcon(); syncSidebarVersion(); syncSettingsCardLayout(); syncSelectedFileList(); syncMockControls(); syncMyboxCredentialStatus(); syncMyboxCredentialEditor(); syncMoreResults(); onboardingController?.refreshTarget(); void syncRuntimeComponents(); };
