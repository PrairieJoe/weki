...
render=function(){ baseRender(); syncBrandIcon(); syncSelectedFileList(); syncMockControls(); syncMyboxCredentialStatus(); syncMyboxCredentialEditor(); syncMoreResults(); void syncRuntimeComponents(); };
uploadFiles=async function(){ if(!state.selectedFiles.length){state.toast="등록할 파일을 먼저 선택해 주세요.";render();return;} const mode=document.querySelector('input[name="mode"]:checked')?.value||"lightweight"; const form=new FormData();state.selectedFiles.forEach((file)=>form.append("files",file));form.append("mode",mode);state.toast="문서를 처리 대기열에 추가하는 중…";render();try{const response=await fetch("/api/documents",{method:"POST",body:form});const data=await response.json();if(response.ok)state.selectedFiles=[];state.toast=response.ok&&data.processing?.fallbackReason==="semantic_model_unavailable"?"Local AI 모델이 준비되지 않아 경량 처리로 전환했습니다. 구성요소를 설치한 뒤 다시 시도해 주세요.":response.ok?"문서를 처리 대기열에 추가했습니다.":data.error||"문서 등록에 실패했습니다.";await refresh();}catch{state.toast="문서 등록 중 네트워크 오류가 발생했습니다.";}render();};
