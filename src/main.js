...
import { runtimeAction, runtimeBatchMessage, runtimeBatchProgress } from "./runtime/presentation.mjs";
import { armRuntimeInstallAutoRestart } from "./runtime/auto-restart.mjs";
import { shouldRenderStatusPoll } from "./search/status-polling.mjs";
...
const runtimeComponentLabels = { "semantic-model":"의미 검색 모델", "semantic-reranker":"검색 결과 재정렬 모델", "document-renderer":"HWP/HWPX 문서 화면 처리기", "presentation-renderer":"Office 문서·PPT 렌더러" };
...
function nav(){
 ...
 async function fetchJsonWithDiagnostics(endpoint, failureMessage){
   let response;
   try { response=await fetch(endpoint); }
   catch(error){ state.toast=`${failureMessage} Weki 로컬 서버에 연결할 수 없습니다. (${error.message||"네트워크 오류"})`; return null; }
   const raw=await response.text();
   let payload={};
   try { payload=raw?JSON.parse(raw):{}; }
   catch { state.toast=`${failureMessage} 서버 응답을 해석할 수 없습니다. (HTTP ${response.status})`; return null; }
   if(!response.ok){ state.toast=`${failureMessage} ${payload.error||`서버 오류(HTTP ${response.status})`}`; return null; }
   return payload;
 }
 refresh=async function refreshWithDiagnostics(){
   const shouldFetchMybox=!state.myboxLoaded||state.page==="settings";
   const shouldFetchSettings=state.page==="settings"||state.page==="add";
   const requests=[
     fetchJsonWithDiagnostics("/api/documents","문서 목록을 불러오지 못했습니다."),
     fetchJsonWithDiagnostics("/api/jobs","작업 목록을 불러오지 못했습니다."),
     fetchJsonWithDiagnostics("/api/status","로컬 상태를 확인하지 못했습니다."),
   ];
   if(shouldFetchMybox)requests.push(fetchJsonWithDiagnostics("/api/mybox/status","MYBOX 상태를 확인하지 못했습니다."));
   if(shouldFetchSettings)requests.push(fetchJsonWithDiagnostics("/api/settings","처리 설정을 불러오지 못했습니다."));
   const responses=await Promise.all(requests);
   if(responses.some((response)=>response===null))return false;
   const docData=responses[0],jobData=responses[1]; state.status=responses[2]; let responseIndex=3;
   if(shouldFetchMybox){state.mybox=responses[responseIndex++];state.myboxLoaded=true;}
   if(shouldFetchSettings){const settingsData=responses[responseIndex];state.settings=settingsData.settings||state.settings;if(settingsData.processing)state.status={...(state.status||{}),processing:settingsData.processing};}
   const semantic=state.status.search?.semantic;
   if(semantic&&typeof semantic==="object"){state.status.search.semanticDetails=semantic;state.status.search.semantic=semantic.status;}
   docs=docData.documents.map((doc)=>({...doc,type:doc.format,source:sourceStatusLabel(doc.sourceStatus),date:doc.registeredAt?new Date(doc.registeredAt).toLocaleDateString("ko-KR"):"-"})); state.jobs=jobData.jobs.slice(0,20); return true;
 };
...
function addPage(){ const failed=state.jobs.filter((job)=>job.status==="failed").length; return `<main>${topbar("INGESTION","문서 등록","파일을 내부 저장소에 보관하고 페이지별 실제 텍스트를 로컬 색인에 추가합니다.")}${failed?banner(`${failed}개 작업이 실패했습니다. 작업 기록에서 원인을 확인하고 재처리하세요.`,"error"):""}<div class="split"><section class="upload-panel" data-onboarding-target="registration-screen"><p class="eyebrow">NEW PROCESSING JOB</p><h2>무엇을 등록할까요?</h2><input id="file-input" type="file" multiple accept=".pdf,.pptx,.docx,.hwpx,.hwp" hidden/><div class="dropzone"><div class="drop-icon" aria-hidden="true">${icons.add}</div><h3>지원 문서를 선택하세요</h3><p>PDF, PPTX, HWP, HWPX, DOCX · 최대 200 MB/파일</p><button class="secondary" id="choose-files" data-onboarding-target="choose-files">파일 선택</button></div><label class="field-label" for="mode-lightweight">처리 모드</label><div class="modes" data-onboarding-target="registration-mode"><label class="mode selected"><input id="mode-lightweight" type="radio" checked value="lightweight" name="mode"/><span><b>경량 처리만</b><small>로컬 Parsing 및 페이지별 텍스트 색인</small></span><em>기본</em></label><label class="mode"><input type="radio" value="local-ai" name="mode"/><span><b>Local AI 허용</b><small>로컬 모델을 사용할 수 있습니다.</small></span></label><label class="mode external"><input type="radio" value="external-ai" name="mode"/><span><b>External AI 허용</b><small>실행 전에 외부 전송 범위를 안내합니다.</small></span></label></div><button class="primary wide" id="create-job">선택 파일 등록 ${icons.arrow}</button></section><section class="queue-panel" data-onboarding-target="processing-queue"><div class="section-title"><div><p class="eyebrow">PROCESSING QUEUE</p><h2>처리 현황 <b>${state.jobs.length}</b></h2></div><span class="queue-state">${activeJobs().length?`${activeJobs().length}개 진행 중`:"대기 없음"}</span></div>${state.jobs.length?state.jobs.map(jobCard).join(""):"<div class=\"empty-state\"><div>✓</div><h2>처리 대기열이 비어 있습니다</h2><p>문서를 등록하면 여기에 작업 진행 상황이 표시됩니다.</p></div>"}<div class="queue-note"><span aria-hidden="true">i</span><p>추출에 성공한 페이지는 Text Evidence와 정확한 페이지 번호를 보존해 검색합니다.</p></div></section></div></main>`; }
...
function syncGeminiCredentialEditor(){
  ...
  const credentialFailure=(action,result)=>{if(result?.encryptionAvailable!==true)return "Windows 보안 저장소를 사용할 수 없어 Gemini Key를 관리할 수 없습니다.";if(action==="save"&&result?.configured!==true)return "Gemini Key 저장에 실패했습니다.";if(action==="clear"&&result?.configured!==false)return "저장된 Gemini Key 삭제에 실패했습니다.";return "";};
  saveButton.addEventListener("click",async()=>{saveButton.disabled=true;clearButton.disabled=true;geminiCredentialMessage="Key를 이 PC에 암호화하여 저장하는 중…";message.textContent=geminiCredentialMessage;try{const result=await window.wekiAiCredentials.saveGeminiKey(draftKey);const failure=credentialFailure("save",result);if(failure){geminiCredentialMessage=failure;message.textContent=geminiCredentialMessage;return;}draftKey="";input.value="";geminiCredentialMessage="Gemini Key를 저장했습니다. 연결 확인은 별도로 실행하세요.";message.textContent=geminiCredentialMessage;await refresh();render();}catch{geminiCredentialMessage="Gemini Key 저장에 실패했습니다.";message.textContent=geminiCredentialMessage;}finally{saveButton.disabled=false;clearButton.disabled=false;}});
  clearButton.addEventListener("click",async()=>{saveButton.disabled=true;clearButton.disabled=true;geminiCredentialMessage="저장된 Gemini Key를 삭제하는 중…";message.textContent=geminiCredentialMessage;try{const result=await window.wekiAiCredentials.clearGeminiKey();const failure=credentialFailure("clear",result);if(failure){geminiCredentialMessage=failure;message.textContent=geminiCredentialMessage;return;}draftKey="";input.value="";state.geminiConnectionStatus=null;geminiCredentialMessage="저장된 Gemini Key를 삭제했습니다. 외부 AI는 설치된 모델, 경량 처리 순으로 fallback합니다.";message.textContent=geminiCredentialMessage;await refresh();render();}catch{geminiCredentialMessage="저장된 Gemini Key 삭제에 실패했습니다.";message.textContent=geminiCredentialMessage;}finally{saveButton.disabled=false;clearButton.disabled=false;}});
}
...
function syncMyboxConnectionLabel(){
  if(state.page!=="settings")return;
  document.querySelectorAll(".settings-card .engine").forEach((row)=>{
    const label=row.querySelector("b");
    if(label?.textContent.trim()!=="MYBOX 연결 상태")return;
    label.textContent="MYBOX 저장공간 연결";
    const status=row.querySelector("em");
    if(status)status.textContent=state.mybox?.message||(state.mybox?.connected?"연결됨":"연결 안 됨");
  });
}
async function installAllRuntimeComponents(button){
  ...
  const pending=["semantic-model","semantic-reranker","document-renderer","presentation-renderer"].filter((id)=>{
  ...
  runtimeInstallRestartDeferred=false;
  button.disabled=true;state.toast="고품질 검색 구성요소를 설치하는 중입니다…";if(state.page===originPage)render();
  startRuntimePolling();
  try{
    ...
    const shouldArmWatcher=state.page===originPage&&!runtimeInstallRestartDeferred;
    const watcher=shouldArmWatcher
      ?await armRuntimeInstallAutoRestart({responseOk:response.ok,startedAt:data.startedAt,watch:window.wekiApp?.watchRuntimeInstall})
      :{ok:false,reason:"restart-deferred-after-navigation"};
    state.toast=response.ok
      ?watcher.ok?"고품질 검색 구성요소 설치를 시작했습니다. 완료되면 앱이 자동으로 다시 시작됩니다.":"설치는 시작했지만 자동 재시작을 보류했습니다. 완료 후 설정에서 앱 다시 시작을 눌러 적용해 주세요."
      :data.error||"고품질 검색 구성요소 설치에 실패했습니다.";
  }catch(error){state.toast=error.message||"고품질 검색 구성요소 설치에 실패했습니다.";}
  if(state.page===originPage)render();
}
function enhanceRuntimeCard(card,data){
  const entries=Object.values(data?.components||{});
  const batchProgress=runtimeBatchProgress(data?.installBatch);
  ...
  if(description){description.classList.add("runtime-description");description.textContent="네 가지 구성요소를 설치하면 의미 기반 검색, Office/PPT 시각 근거와 검색 결과 정렬 품질이 향상됩니다. 필요한 항목만 설치하거나 전체 설치를 선택할 수 있습니다.";}
  ...
  rows.forEach((row,index)=>{
    ...
    const action=batchProgress?.componentId===entry.id?{label:"진행 중",disabled:true}:runtimeAction(entry,metadata);
  ...
  card.querySelector(".runtime-batch-progress")?.remove();
  if(batchProgress){
    const progress=document.createElement("div");
    progress.className="runtime-progress runtime-batch-progress is-indeterminate";
    const componentLabel=runtimeComponentLabels[batchProgress.componentId]||batchProgress.componentId;
    progress.setAttribute("role","group");progress.setAttribute("aria-label",`${componentLabel} 설치 진행 상황`);
    progress.innerHTML=`<b>${escape(componentLabel)} 설치 중</b><span>구성요소 ${batchProgress.currentStep}/${batchProgress.total} · 다운로드 및 적용 중</span><i role="progressbar" aria-label="${escape(componentLabel)} 설치 진행 중" aria-valuemin="0" aria-valuemax="100" aria-valuetext="진행률 확인 중"><em></em></i>`;
    card.querySelector("#document-renderer-install-slot")?.before(progress);
  }
...
function syncRuntimeCardPresentation(card,data){
  ...
  for(const entry of entries){
    ...
    if(label)label.textContent=runtimeComponentLabels[entry.id]||entry.id;
    const isBatchCurrent=batchProgress?.componentId===entry.id;
    const dot=row.querySelector(":scope > span");
    if(dot&&isBatchCurrent)dot.className="processing-dot";
    if(status){const statusText=isBatchCurrent?`설치 진행 중 · ${batchProgress.currentStep}/${batchProgress.total}`:runtimeStatusText(entry),source=entry.requiresMybox&&statusText!=="MYBOX 배포본 없음"?" · MYBOX 배포본":"";status.textContent=`${statusText}${source}`;}
  }
  ...
  const installing=entries.some((entry)=>entry.status==="installing")||Boolean(batchProgress);
...
async function installRuntimeComponent(button,{componentId,version,source=null,manifestUrl=null}){
  const originPage=state.page;
  const confirmation=source==="mybox"?`MYBOX runtime manifest에서 ${componentId==="document-renderer"?"HWP/HWPX 문서 화면 처리기":"Office 문서·PPT 렌더러"}를 설치할까요?`:`${componentId} 구성요소를 다운로드할까요?`;
  ...
  finally{if(isRenderer)state.myboxRendererInstalling=false;if(state.page===originPage)render();}
}
...
function applyMyboxRuntimeAction(card,result){
  ...
  if(runtime.status&&runtime.status!=="ready"&&!runtime?.structure?.manifest){const labels={runtime_root_missing:"MYBOX에 wiki 폴더가 없습니다.",runtime_folder_missing:"MYBOX에 runtime 폴더가 없습니다.",runtime_version_missing:"MYBOX에 runtime/v1 폴더가 없습니다.",manifest_missing:"MYBOX에 renderer 배포 manifest가 없습니다.",manifest_unreadable:"MYBOX renderer manifest를 읽을 수 없습니다."};if(!fallback())note(labels[runtime.status]||"MYBOX runtime 배포 상태를 확인할 수 없습니다.");return;}
...
async function syncRuntimeComponents(){
  ...
  if(!card){ card=document.createElement("section"); card.id="runtime-components-card"; card.className="settings-card span2"; grid.prepend(card); }
   if(state.runtime){updateCard(state.runtime);syncRuntimeCardPresentation(card,state.runtime);syncSettingsCardLayout();void syncMyboxRuntimeAction(card);}
  if(state.runtimeLoading)return;
   state.runtimeLoading=true; try{const data=await (await fetch("/api/runtime/components")).json(); state.runtime=data; updateCard(data); syncRuntimeCardPresentation(card,data); syncSettingsCardLayout(); void syncMyboxRuntimeAction(card); if(Object.values(data.components||{}).some((entry)=>entry.status==="installing")||runtimeBatchProgress(data.installBatch))startRuntimePolling();}catch{} finally{state.runtimeLoading=false;}
}
function syncSettingsCardLayout(){ if(state.page!=="settings")return; const grid=document.querySelector(".settings-grid"); if(!grid)return; const orderByEyebrow={"OPTIONAL RUNTIME PACKS":0,"LOCAL STORAGE":1,"AI DEFAULT":2,"RETRIEVAL":3,"EXTERNAL AI":4,"MYBOX CLOUD":5,"SYNONYMS":6,"AUDIT":7,"DATA":8}; grid.querySelectorAll(".settings-card").forEach((card)=>{const label=card.querySelector(".eyebrow")?.textContent.trim();if(label in orderByEyebrow)card.style.order=String(orderByEyebrow[label]);}); }
function syncSidebarVersion(){ const sidebar=document.querySelector(".sidebar"),target=sidebar?.querySelector(".sidebar-bottom"); if(!target||target.querySelector(".app-version"))return; const version=document.createElement("small"); version.className="app-version"; version.textContent=`v${APP_VERSION}`; target.prepend(version); }
...
function searchComposerIsActive(){ return state.page==="search"&&document.activeElement?.id==="query"; }
function reportSearchInputEvent(type){ try{window.wekiApp?.reportSearchInput?.({type,activeElementId:document.activeElement?.id||"",hasFocus:document.hasFocus(),visibility:document.visibilityState});}catch{} }
function bindSearchComposerFocus(input){ if(!input||input.dataset.focusBound==="true")return; input.dataset.focusBound="true"; input.addEventListener("pointerdown",()=>{reportSearchInputEvent("pointerdown");try{const focus=window.wekiApp?.focusWindow?.();focus?.catch?.(()=>{});}catch{} input.focus({preventScroll:true});}); input.addEventListener("focus",()=>reportSearchInputEvent("focus")); input.addEventListener("blur",()=>reportSearchInputEvent("blur")); input.addEventListener("input",()=>reportSearchInputEvent("input")); }
function focusSearchComposer(){ if(state.page!=="search"||document.querySelector("#onboarding-root")){searchWindowFocusRequested=false;return;} const input=document.querySelector("#query"); if(!input)return; bindSearchComposerFocus(input); if(!searchWindowFocusRequested){searchWindowFocusRequested=true;try{const focus=window.wekiApp?.focusWindow?.();focus?.catch?.(()=>{});}catch{}} input.focus({preventScroll:true}); const end=input.value.length; input.setSelectionRange?.(end,end); }
function deferRuntimeInstallRestart(){
  runtimeInstallRestartDeferred=true;
  if(state.toast.includes("고품질 검색 구성요소"))state.toast="구성요소 설치 중에는 자동 재시작을 보류합니다. 완료 후 설정에서 앱 다시 시작을 눌러 적용해 주세요.";
  try{ const cancellation=window.wekiApp?.cancelRuntimeInstallWatch?.(); cancellation?.catch?.(()=>{}); }catch{}
}
async function navigateFromNav(targetPage){
  if(state.page===targetPage)return;
  if(targetPage!=="settings")deferRuntimeInstallRestart();
  state.page=targetPage;
  render();
  await refresh().catch(()=>{});
  if(state.page===targetPage&&!searchComposerIsActive())render();
}
const baseRender=render;
async function navigateOnboardingPage(nextPage){ if(state.page===nextPage)return; if(nextPage!=="settings")deferRuntimeInstallRestart(); state.page=nextPage; await refresh().catch(()=>{}); render(); }
async function restoreOnboardingPage(previousPage){ if(state.page===previousPage)return; if(previousPage!=="settings")deferRuntimeInstallRestart(); state.page=previousPage; await refresh().catch(()=>{}); render(); }
render=function({force=false}={}){ if(!force&&searchComposerIsActive())return; baseRender(); syncBrandIcon(); syncSidebarVersion(); syncSettingsCardLayout(); syncSelectedFileList(); syncMockControls(); syncMyboxCredentialStatus(); syncMyboxCredentialEditor(); syncMoreResults(); onboardingController?.refreshTarget(); void syncRuntimeComponents(); focusSearchComposer(); };
...
document.addEventListener("click",(event)=>{const nav=event.target.closest?.("[data-nav]");if(nav&&nav.dataset.nav!=="settings")deferRuntimeInstallRestart();},true);
...
let statusPollSignature = "";
function scheduleStatusPoll(delay = statusPollDelay) {
  if (statusPollTimer) clearTimeout(statusPollTimer);
  statusPollTimer = setTimeout(async () => {
    statusPollTimer = null;
    if (document.querySelector("#onboarding-root")) {
      statusPollDelay = Math.min(5000, statusPollDelay + 500);
      scheduleStatusPoll();
      return;
    }
    const trackingSync = state.page === "settings" && state.mybox?.sync?.state === "syncing";
    const shouldRefresh = !state.loading && ((state.page !== "add" && !trackingSync) || (state.page === "add" && activeJobs().length > 0));
    if (!shouldRefresh || refreshInFlight) { statusPollDelay = Math.min(5000, statusPollDelay + 500); scheduleStatusPoll(); return; }
    const before = statusPollSignature;
    refreshInFlight = true;
    try {
      await refresh();
      if (shouldRenderStatusPoll(state.page)) render();
      const next = JSON.stringify((state.jobs || []).slice(0, 20).map((job) => [job.id, job.status, job.progress, job.completedUnits]));
      statusPollSignature = next;
      const active = activeJobs().length > 0;
      statusPollDelay = next !== before ? (active ? 500 : 1000) : Math.min(active ? 2500 : 5000, Math.max(500, statusPollDelay * 1.5));
    } catch { statusPollDelay = Math.min(5000, statusPollDelay * 1.5); }
    finally { refreshInFlight = false; scheduleStatusPoll(); }
  }, Math.max(250, delay));
}
scheduleStatusPoll();
// Legacy contract retained for static checks: setInterval(async () => { if (activeJobs().length) { await refresh(); render(); } });
