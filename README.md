...
- 백업은 MYBOX만 지원합니다. v1.3.0은 v1.2.0에서 생성한 로컬 `.weki` 백업을 생성하거나 복원하지 않습니다. 기존 `.weki` 파일은 자동 삭제하지 않지만 복원 대상으로 취급하지 않습니다.
- 사용자 설정 화면의 검색 색인 관리 기능을 제거했습니다. 내부 검색 색인 재생성 API는 마이그레이션·복구용으로만 유지하며, 사용자에게 노출하지 않습니다.
- 전체 문서 재처리(파싱·OCR·AI·Embedding·색인 재실행)는 이번 v1.3.0 범위에 포함하지 않습니다.
- 일반 화면에는 앱 릴리즈 버전만 표시합니다. runtime/ranking 내부 버전은 진단 용도로만 유지합니다.
- 첫 화면이 준비된 뒤 완료 표식이 없는 설치 프로필에서 선택형 9단계 온보딩 투어를 한 번 안내합니다. 순서는 `문서 등록 화면 → 파일 선택 → 처리 모드 → 처리 대기열 → 문서 관리 → 질문 검색 → 검색 결과 예시 → 근거 예시 → MYBOX와 저장소`입니다. 실제 문서·검색 결과가 아닌 정적 예시를 포함하며, 투어에서는 실제 문서나 설정을 변경하지 않습니다.
- 설정·운영 화면의 고품질 검색 구성요소는 구성요소명·상태·작업 열을 고정해 설치 버튼 유무와 관계없이 행을 정렬합니다.
- 기존 `사용 가이드`를 선택하면 언제든 1단계부터 다시 볼 수 있습니다. `건너뛰기`·닫기·완료 후에는 투어를 시작한 화면으로 돌아갑니다. 이미 완료한 사용자는 자동으로 다시 재생되지 않으며, 필요할 때만 `사용 가이드`로 재생합니다.
- 온보딩 투어는 문서를 등록하거나 검색·설정 변경·MYBOX 업로드·동기화를 수행하지 않습니다. 완료 여부는 제품 데이터와 분리된 이 설치 프로필의 로컬 `localStorage`에만 기록합니다.
- 투어는 상시 hover tooltip이 아니라 단계형 코치 마크이며, 데스크톱에서는 제목을 드래그해 대화상자를 이동할 수 있고 화면 안쪽으로 자동 제한합니다. 모바일에서는 대화상자를 중앙에 유지합니다. 대화상자 내부 포커스·Tab 이동·Esc 종료와 종료 후 포커스 복귀도 지원합니다.
- 설정의 기본 처리 모드는 `설치된 모델 사용`과 `외부 AI 사용` 두 가지입니다. 문서별 등록 화면에서는 `경량 처리만`, `Local AI 허용`, `External AI 허용` 세 가지를 선택할 수 있습니다. `설치된 모델 사용`(`auto`)은 세 runtime 구성요소가 모두 준비·적용되기 전에는 경량 처리로 동작하고, 준비되면 새 작업부터 Local AI를 사용합니다. 기존 문서는 자동 재처리하지 않습니다.
- `External AI 허용`은 Gemini를 문서 전처리 보조 및 검색 보조 색인에 사용하는 상한입니다. Gemini Key는 이 PC에 암호화해 저장하고 화면에 다시 표시하지 않습니다. 외부 AI를 끄면 신규 작업부터 외부 처리가 차단되며, 이미 등록된 Queue 작업의 snapshot 정책은 바뀌지 않습니다.
- 외부 전송은 필요한 페이지·슬라이드의 추출 텍스트와 선택한 이미지로 제한합니다. 텍스트는 최대 12,000자, 이미지는 최대 4개·이미지당 2 MiB·총 8 MiB이며 요청 제한 시간은 30초입니다. 전체 원본 파일을 전송하지 않으며, 외부 AI가 생성한 summary·topic·keywords·visual description은 검색 보조 메타데이터일 뿐 원문 Evidence를 대체하지 않습니다. 최종 답변 생성은 범위에 포함하지 않습니다.
- `External AI 허용`을 켜는 모든 OFF→ON 전환에는 동의 확인이 필요합니다. Key·모델이 없거나 연결이 실패하면 `External AI → Local AI → 경량 처리` 순으로 허용된 fallback을 사용합니다.

자세한 변경 사항은 [v1.3.0 릴리스 노트](./docs/RELEASE_NOTES_V1.3.0.md)를 참조하세요.
...
- 설정 · 운영 화면에서 의미 검색 모델, 검색 결과 재정렬 모델, 문서 화면 처리기, Office 문서·PPT 렌더러의 4개 상태를 항상 확인할 수 있습니다. 배포본이 있는 구성요소만 `전체 설치`로 순서대로 설치하며, 버전·업데이트·앱 재시작 필요 여부와 MYBOX 배포본 여부를 표시합니다.
- 기본 처리 모드는 `설치 상태에 따라 자동`으로 설정할 수 있습니다. 의미 검색 모델·재정렬 모델·문서 화면 처리기의 3개 구성요소가 모두 준비되어 적용되기 전에는 경량 처리로 동작하고, 모두 준비되면 새 문서부터 Local AI를 기본으로 사용할 수 있습니다. Office 문서·PPT 렌더러는 DOCX/PPTX 페이지·슬라이드 품질을 위한 별도 구성요소입니다.
...
- 관련 설계·운영 문서는 [vNext 1단계 검색 문서](./docs/WEKI_VNEXT_STAGE1.md), [vNext 2단계 구성요소 문서](./docs/WEKI_VNEXT_STAGE2.md), [v1.3.0 릴리스 노트](./docs/RELEASE_NOTES_V1.3.0.md), [이용자 테스트 체크리스트](./docs/USER_TEST_CHECKLIST.md), [변경 이력](./docs/CHANGELOG.md)에서 확인할 수 있습니다.
...
생성물은 `release/Weki-1.3.0-Setup.exe`입니다.

## 현재 지원

- PDF, DOCX, PPTX, HWPX, HWP 5.x의 로컬 텍스트 추출
- PDF 전체 페이지 로컬 OCR, PPTX 내 PNG/JPEG/WebP 시각 자산 OCR
- LibreOffice Portable runtime이 준비되면 DOCX/PPTX를 실제 페이지·슬라이드로 렌더링한 뒤 OCR하며, PPTX native 표·차트 구조 추출과 병합합니다.
- `@rhwp/core` runtime은 HWP/HWPX 전용 renderer로 유지하고, LibreOffice와 별도 구성요소로 설치·적용합니다.
...
- LibreOffice runtime이 설치되지 않았거나 렌더링에 실패한 DOCX/PPTX는 native parser와 내장 이미지 OCR fallback으로 처리합니다.
- HWPX/HWP의 시각 요소는 HWP/HWPX 전용 renderer가 없거나 실패하면 추출 가능한 원문 텍스트와 OCR fallback 중심으로 처리합니다.
- HWP는 파서가 지원하는 HWP 5.x 구조에 한정됩니다.
- 문서 등록의 `Local AI 허용`은 semantic model runtime pack이 적용된 환경에서 사용할 수 있습니다. 기본 처리 모드를 `설치된 모델 사용`으로 두면 세 runtime 구성요소가 모두 준비·적용되기 전까지 경량 처리로 동작하고, 준비되면 새 작업부터 Local AI를 사용합니다. 모델이 없거나 적용되지 않은 명시적 Local AI 요청은 경량 처리로 전환합니다. `External AI 허용`은 Gemini를 전처리 보조와 검색 보조 색인에만 사용하며, 외부 AI는 최종 답변을 생성하지 않습니다.
...
- MYBOX 원본은 `weki/data/<파일 Hash>/<원본 파일명>`에 저장합니다. `knowledge-base.json`은 `data/<파일 Hash>/<원본 파일명>`만 참조합니다. v1.3.0은 기존 flat/root `data` 구조와 v1.2.0의 단일 `.weki` 파일을 자동 호환·이동·삭제하지 않으며, `.weki` 복원을 지원하지 않습니다.
