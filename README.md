...
- 백업은 MYBOX만 지원합니다. v1.2.3은 v1.2.0에서 생성한 로컬 `.weki` 백업을 생성하거나 복원하지 않습니다. 기존 `.weki` 파일은 자동 삭제하지 않지만 복원 대상으로 취급하지 않습니다.
...
자세한 변경 사항은 [v1.2.3 릴리스 노트](./docs/RELEASE_NOTES_V1.2.3.md)를 참조하세요.
...
- 관련 설계·운영 문서는 [vNext 1단계 검색 문서](./docs/WEKI_VNEXT_STAGE1.md), [vNext 2단계 구성요소 문서](./docs/WEKI_VNEXT_STAGE2.md), [v1.2.3 릴리스 노트](./docs/RELEASE_NOTES_V1.2.3.md), [이용자 테스트 체크리스트](./docs/USER_TEST_CHECKLIST.md), [변경 이력](./docs/CHANGELOG.md)에서 확인할 수 있습니다.
...
생성물은 `release/Weki-1.2.3-Setup.exe`입니다.
...
- MYBOX 원본은 `weki/data/<파일 Hash>/<원본 파일명>`에 저장합니다. `knowledge-base.json`은 `data/<파일 Hash>/<원본 파일명>`만 참조합니다. v1.2.3은 기존 flat/root `data` 구조와 v1.2.0의 단일 `.weki` 파일을 자동 호환·이동·삭제하지 않으며, `.weki` 복원을 지원하지 않습니다.
